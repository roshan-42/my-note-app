...
export default function QuestionEditorModal({
  ...
  const [type, setType] = useState<"past" | "possible">(
    question?.type || "past",
  );
    ...
    <AnimatePresence>
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-start sm:items-center justify-center p-0 sm:p-4 overflow-y-auto">
        ...
        >
            ...
            {(["past", "possible"] as const).map((t) => (
              <label key={t} className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  checked={type === t}
                  onChange={(e) =>
                    setType(e.target.value as "past" | "possible")
                  }
                  disabled={saving}
                  className="w-4 h-4 accent-[var(--accent)]"
                />
                <span className="text-sm font-medium text-[var(--text-2)]">
                  {t === "past" ? "Past Question" : "Possible Question"}
                </span>
              </label>
            ))}
          </div>
          ...
          <div className="overflow-y-auto max-h-96  flex-1 p-4 sm:p-6 space-y-5">
            {activeLang === "en" ? (
              <>
          ...
          <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-2 sm:gap-3 px-4 sm:px-6 py-3 sm:py-4 border-t border-[var(--border)] bg-[var(--bg-1)]">
            ...
            <div className="flex gap-2">
              <button
                onClick={onClose}
                disabled={saving}
                className="flex-1 sm:flex-initial px-4 py-2 rounded-lg border border-[var(--border)] text-[var(--text-2)] hover:bg-[var(--surface-2)] disabled:opacity-50"
              >
                Cancel
              </button>
              <button
