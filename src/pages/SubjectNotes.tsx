...
export default function SubjectNotes() {
  ...
  const chaptersWithNotes = useMemo(() => [...chapters]
  ...
  useEffect(() => {
    if (!chSlug && chaptersWithNotes.length > 0) {
      navigate(`/faculty/${facSlug}/year/${yearNum}/${subjSlug}/notes/${chaptersWithNotes[0].slug}`, { replace: true });
    }
    ...
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-4rem)]">
      ...
      <aside className={`fixed lg:sticky lg:top-16 top-0 left-0 w-80 border-r border-[var(--border)] bg-[var(--bg-1)] h-screen lg:h-[calc(100vh-4rem)] overflow-y-auto z-40 transition-transform duration-300 ${
        ...
        <div className="p-5 border-b border-[var(--border)] sticky top-0 bg-[var(--bg-1)] z-10">
          ...
          <p className="text-xs text-[var(--text-3)] mt-0.5">{chaptersWithNotes.length} total</p>
        </div>
            ...
            <div key={ch.id}>
              ...
              {expanded.has(ch.id) && (
                <div className="ml-7 mt-1 space-y-0.5 pb-2">
                  {ch.notes.length === 0 ? (
                    <div className="text-xs px-3 py-1 text-[var(--text-3)] italic">No notes</div>
                  ) : (
                    ch.notes.map(n => (
                      <div key={n.id} className="text-xs px-3 py-1.5 rounded text-[var(--text-3)] truncate">
